sap.ui.controller("WebContent.newstile.NewsTileDrillDown", {
    ...
    _focusSelectedNewsItem : function(channelId, eventId, data) {
        ...
        // Store the Main NewsTile items index to apply selection on DrillDown NewsItem List
     ...
     image : function(value) {
        ...
        var bDisplayArticleImage = newstile.NewsTileUtils.getUseDefaultImageConfig(oChipApi);
        if (value) {
            return value;
        } else {
            var oDefaultImage = newstile.NewsTileUtils.getDefaultImageConfig(oChipApi);
            if (!oDefaultImage) {
                oDefaultImage = newstile.NewsTileUtils.getFinalDefaultImageForDrillDown(oChipApi);
            }
